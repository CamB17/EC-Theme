...
add_action('admin_init', 'ec_remove_dashboard_widgets');
function ec_remove_dashboard_widgets() {
...
// legacy remove meta
add_action( 'admin_menu' , 'ec_remove_metaboxes' );
function ec_remove_metaboxes() {
...
// Remove Smilies
add_filter('option_use_smilies','ec_remove_smileys',99,1);
function ec_remove_smileys($bool) {
  return false;
}
...
// add_filter('tiny_mce_before_init', 'ec_custom_tinymce' );
function ec_custom_tinymce($init) {
...
add_action('init', 'ec_rm_headlink');
function ec_rm_headlink() {
  remove_action('wp_head', 'rsd_link');
  remove_action('wp_head', 'wlwmanifest_link');
  remove_action('wp_head', 'index_rel_link');
...
add_filter( 'next_post_rel_link', 'ec_disable_stuff' );
function ec_disable_stuff( $data ) {
  return false;
}

// Remove Widgets
add_action('widgets_init', 'ec_unregister_default_wp_widgets', 1);
function ec_unregister_default_wp_widgets() {
...
function ec_remove_sticky_class($classes) {
...
add_filter('post_class','ec_remove_sticky_class');
