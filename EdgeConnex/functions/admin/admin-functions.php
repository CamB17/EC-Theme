...
function ec_custom_admin_footer() {
  _e('<span id="footer-thankyou">Developed by <a href="http://www.cameronbarclay.com" target="_blank">AOR</a></span>.', 'aor');
}
add_filter('admin_footer_text', 'ec_custom_admin_footer');

// Login css
function ec_login_css() {
  wp_enqueue_style( 'ec_login_css', get_template_directory_uri() . '/styles/login.css', false );
}
add_action( 'login_enqueue_scripts', 'ec_login_css', 10 );

// changing the logo link from wordpress.org to your site
function ec_login_url() {  return home_url(); }
add_filter('login_headerurl', 'ec_login_url');

// changing the alt text on the logo to show your site name
function ec_login_title() { return get_option('blogname'); }
add_filter('login_headertitle', 'ec_login_title');

// remove the default option in the page template selector
function ec_remove_default_template_option() {
    global $pagenow;
...
add_action('admin_footer', 'ec_remove_default_template_option', 10);

//allow extra file types in uploader
function ec_allow_extra_file_types($mimes) {
  $mimes['svg'] = 'image/svg+xml';
  return $mimes;
}
add_filter('upload_mimes', 'ec_allow_extra_file_types');

//add meta box to expand and contract the acf fields
function ec_expand_acf_box() {
  ?>
...
function ec_add_custom_meta_boxes() {
    add_meta_box("expand_acf_box", "Expand & Contract ACF Fields", "ec_expand_acf_box", "acf-field-group", "side", "high", null);
    add_meta_box("expand_pb_box", "Expand & Contract Page Builder", "ec_expand_pb_box", "page", "side", "high", null);
}
add_action("add_meta_boxes", "ec_add_custom_meta_boxes");

// default permalink structure
// function ec_set_permalinks() {
//     global $wp_rewrite;
//     $wp_rewrite->set_permalink_structure( '/%postname%/' );
// }
// add_action( 'init', 'ec_set_permalinks' );

// duplicate post plugin option set
update_option( 'duplicate_post_copystatus', '1' );


function ec_edit_per_page( $result, $option, $user ) {
    if ( (int)$result < 1 )
        return 200;
}
// automatically set the default posts per page for every custom post
function ec_set_default_per_page() {
  $postTypes = get_post_types( array('public' => 'true', '_builtin' => 'false'), 'names','or');
  foreach ( $postTypes as $postType ) {
    add_filter( 'get_user_option_edit_' . $postType . '_per_page', 'ec_edit_per_page', 10, 3 );
  }
}
add_action( 'admin_head', 'ec_set_default_per_page', 10 );
